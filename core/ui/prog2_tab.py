...
class Prog2Tab:
    ...
    def criar_topo(self, parent):
        ...
        linha_principal = ttk.Frame(frame_topo)
        linha_principal.grid(row=0, column=0, sticky="ew")

        ttk.Label(linha_principal, text="Buscar", style="Hint.TLabel").grid(row=0, column=0, sticky="w", padx=(0, 5))
        self.entrada_busca = ttk.Entry(linha_principal, textvariable=self.busca_var, width=42)
        self.entrada_busca.grid(row=0, column=1, sticky="ew", padx=(0, 10))
        self.entrada_busca.bind("<KeyRelease>", lambda event: self.refresh())

        self.label_pedidos = ttk.Label(linha_principal, text="Pedidos: 0", style="SummaryValue.TLabel")
        self.label_pedidos.grid(row=0, column=2, sticky="w", padx=(0, 10))

        self.label_itens = ttk.Label(linha_principal, text="Itens: 0", style="SummaryValue.TLabel")
        self.label_itens.grid(row=0, column=3, sticky="w", padx=(0, 10))

        self.label_itens_pendentes = ttk.Label(linha_principal, text="Pend.: 0", style="SummaryValue.TLabel")
        self.label_itens_pendentes.grid(row=0, column=4, sticky="w", padx=(0, 10))

        self.label_total = ttk.Label(linha_principal, text="Total: R$ 0,00", style="SummaryValue.TLabel")
        self.label_total.grid(row=0, column=5, sticky="w", padx=(0, 10))

        self.label_liberado = ttk.Label(linha_principal, text="Liberado: R$ 0,00", style="SummaryValue.TLabel")
        self.label_liberado.grid(row=0, column=6, sticky="w", padx=(0, 10))
        ...
        linha_metas = ttk.Frame(frame_topo)
        linha_metas.grid(row=1, column=0, sticky="ew", pady=(5, 0))

        ttk.Label(linha_metas, text="Meta dia", style="Hint.TLabel").grid(row=0, column=0, sticky="w", padx=(0, 5))
        entrada_dia = ttk.Entry(linha_metas, textvariable=self.meta_faturamento_var, width=13)
        entrada_dia.grid(row=0, column=1, sticky="w", padx=(0, 10))

        ttk.Label(linha_metas, text="Meta mês", style="Hint.TLabel").grid(row=0, column=2, sticky="w", padx=(0, 5))
        entrada_mes = ttk.Entry(linha_metas, textvariable=self.meta_faturamento_mes_var, width=13)
        entrada_mes.grid(row=0, column=3, sticky="w", padx=(0, 12))
        ...
        self.label_meta_resumo.grid(row=0, column=4, sticky="w", padx=(0, 8))

        acoes = ttk.Frame(linha_metas)
        acoes.grid(row=0, column=5, sticky="e")

        self.criar_menu_acoes_prog2(acoes)
        self.criar_menu_exportacao(acoes)

        for _, variavel in (("Dia", self.meta_faturamento_var), ("Mês", self.meta_faturamento_mes_var)):
        ...
        linha_principal.columnconfigure(1, weight=1)
        linha_metas.columnconfigure(4, weight=1)

    def criar_menu_acoes_prog2(self, parent):
        ...
        menu.add_command(label="Desmarcar pedidos", command=self.desmarcar_todos_pedidos)
        menu.add_command(label="Remover marcados", command=self.remover_pedidos_marcados)
        menu.add_separator()
        menu.add_command(label="Recolher todos", command=self.recolher_todos)
        menu.add_command(label="Expandir todos", command=self.expandir_todos)
        menu.add_separator()
        menu.add_command(label="Limpar PROG 2", command=self.controller.limpar_prog2)
        menu_button["menu"] = menu
        menu_button.pack(side="left", padx=(0, 4))

    def criar_menu_exportacao(self, parent, texto=None, comando_excel=None, comando_pdf=None):
        menu_button = ttk.Menubutton(parent, text="Exportar ▾")
        menu = tk.Menu(
            menu_button,
            tearoff=0,
            font=(FONTE, 9),
        ...
        menu.add_command(
            label="Itens liberados - Excel",
            command=lambda: self.controller.exportar_prog2_itens_liberados("excel"),
        )
        menu.add_command(
            label="Itens liberados - PDF",
            command=lambda: self.controller.exportar_prog2_itens_liberados("pdf"),
        )
        menu.add_separator()
        menu.add_command(
            label="Pedidos liberados - Excel",
            command=lambda: self.controller.exportar_prog2_pedidos_liberados("excel"),
        )
        menu.add_command(
            label="Pedidos liberados - PDF",
            command=lambda: self.controller.exportar_prog2_pedidos_liberados("pdf"),
        )
        menu_button["menu"] = menu
    ...
    def criar_tabela(self, parent):
        frame_tabela = ttk.LabelFrame(parent, text="Pedidos e itens selecionados", padding=(7, 5), style="Section.TLabelframe")
        ...
        ttk.Label(
            barra_tabela,
            text="Botão direito abre ações do pedido/item. Pendências são aplicadas no item.",
        ...
        ttk.Button(acoes_tabela, text="Expandir", command=self.expandir_todos, style="Compact.TButton").pack(side="left")
        ...
        self.tabela.column("#0", width=470, minwidth=330, anchor="w", stretch=True)

        larguras = {
            "Sel.": 48,
            "Cliente": 285,
            "Valor Total Pedido": 118,
            "Valor Bloqueado": 118,
            "Valor Liberado": 118,
            "Qtd. Itens": 72,
            "Qtde Item": 80,
        ...
        for coluna in colunas:
            self.tabela.heading(coluna, text=textos_cabecalho.get(coluna, coluna), command=lambda c=coluna: self.alternar_ordenacao(c))
            self.tabela.column(
                coluna,
                width=larguras.get(coluna, 120),
                minwidth=45,
                anchor="center" if coluna in ("Sel.", "Qtd. Itens", "Qtde Item") else "w",
                stretch=False,
            )
