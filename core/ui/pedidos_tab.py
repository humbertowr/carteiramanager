...
class PedidosTab:
    ...
    def criar_area_filtros(self, parent):
        linha_principal = ttk.Frame(parent)
        ...
        ttk.Label(linha_principal, text="Status", style="Hint.TLabel").grid(row=0, column=2, sticky="w", padx=(0, 5))
        ...
        combo_status.grid(row=0, column=3, sticky="w", padx=(0, 10))
        combo_status.bind("<<ComboboxSelected>>", lambda event: self.refresh())

        ttk.Label(linha_principal, text="Grupo", style="Hint.TLabel").grid(row=0, column=4, sticky="w", padx=(0, 5))
        ...
        combo_grupo.grid(row=0, column=5, sticky="w", padx=(0, 10))
        combo_grupo.bind("<<ComboboxSelected>>", lambda event: self.refresh())

        ttk.Label(linha_principal, text="Valor mín.", style="Hint.TLabel").grid(row=0, column=6, sticky="w", padx=(0, 5))
        entrada_valor = ttk.Entry(linha_principal, textvariable=self.valor_minimo_var, width=10)
        entrada_valor.grid(row=0, column=7, sticky="w", padx=(0, 10))
        ...
        self.label_marcados = ttk.Label(
            linha_principal,
            text="0 pedidos marcados",
            style="CardHint.TLabel",
        )
        self.label_marcados.grid(row=0, column=9, sticky="e", padx=(8, 8))
        ...
        self.criar_menu_mais_acoes(linha_principal)

        parent.columnconfigure(0, weight=1)
        linha_principal.columnconfigure(1, weight=1)

    def criar_area_acoes(self, parent):
        linha_secundaria = ttk.Frame(parent)
        linha_secundaria.grid(row=1, column=0, sticky="ew", pady=(5, 0))

        ttk.Label(
            linha_secundaria,
            text="Botão direito abre ações rápidas. Marque pedidos pela coluna Sel.",
            style="Subtitle.TLabel",
        ).grid(row=0, column=0, sticky="w")

        bloco_item = ttk.Frame(linha_secundaria)
        bloco_item.grid(row=0, column=1, sticky="e")
    ...
    def criar_menu_mais_acoes(self, parent):
        ...
        menu.add_separator()
        menu.add_command(label="Bloqueio de itens", command=self.controller.abrir_janela_bloqueio_item)
        menu.add_command(label="Bloqueio de clientes", command=self.controller.abrir_janela_bloqueio_cliente)
        menu.add_command(label="Bloqueio de observações", command=self.controller.abrir_janela_bloqueio_observacao)
        menu_button["menu"] = menu
        menu_button.grid(row=0, column=11, sticky="e")
    ...
    def criar_tabela(self, parent):
        ...
        for coluna in colunas:
            ...
            self.tabela.column(
                coluna,
                width=larguras.get(coluna, 110),
                minwidth=45,
                anchor="center" if coluna in ("Qtde Saldo", "Grupo") else "w",
                stretch=False,
            )
